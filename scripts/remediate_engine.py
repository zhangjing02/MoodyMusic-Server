...
ACC_08 = r2_cfg["account_08"]
s3_client_08 = boto3.client(
    "s3",
    endpoint_url=ACC_08["endpoint_url"],
    aws_access_key_id=ACC_08["access_key_id"],
    aws_secret_access_key=ACC_08["secret_access_key"],
    region_name="auto",
    config=Config(signature_version="s3v4")
)
B08_NAME = ACC_08["name"]
B08_DOMAIN = ACC_08["public_url"].rstrip("/")
...
def download_youtube_track(artist: str, title: str, search_title: str, album: str, out_raw: str) -> bool:
    """通过 YouTube 搜索并采录录音室官方音轨"""
    candidates = []
    for s_t in [search_title, title]:
        if not s_t: continue
        clean_t = re.sub(r'[\(（].*?[\)）]', '', s_t).strip()
    ...
    for q in search_queries:
        ...
        try:
            res = subprocess.run(
                ["yt-dlp", "--proxy", PROXY_URL, "--get-title", "--get-id", f"ytsearch3:{q}"],
            ...
            if len(lines) >= 2:
                for i in range(0, len(lines)-1, 2):
                    ...
                    if core1 in clean_text(vtitle) or core2 in clean_text(vtitle):
                        chosen_vid = vid
                        break
                if not chosen_vid and len(lines) >= 2:
                    chosen_vid = lines[1]
...
def process_single_song(item: dict, work_dir: str) -> dict:
    ...
    try:
        ...
        if r_chk.status_code == 200:
            ...
            if "pub-dd32e05660c74c3dba04d231391eb82b.r2.dev" in f_path:
                print(f"   ⚡ [跳过] 该曲目已在 Bucket 08 点亮合规母带: {f_path}")
    ...
    success = download_youtube_track(artist, title, search_title, album, raw_mp3)
    ...
    matched_phrases = [cl for cl in lrc_clean_lines if cl in simp_heard]
    matched_titles = [t for t in titles_to_check if t in simp_heard]
    
    if len(matched_phrases) >= 1:
        verified = True
        print(f"   ✅ [质检通过] 命中歌词匹配句: {matched_phrases[:2]}")
        ...
        # 二次切片（75s-105s 副歌段复查）
        if dur > 110:
            print(f"   🔄 [二次听音] 启动 75s-105s 副歌段切片复查...")
...
def run_package_pipeline(pkg_json_path: str, work_name: str):
    ...
    for idx, item in enumerate(targets, 1):
        ...
        if res.get("status") == "SUCCESS":
            ...
            if len(d1_batch) >= 3:
                try:
                    requests.post(D1_LIGHT_URL, json={"updates": d1_batch}, timeout=20)
                    print(f"   ⚡ [D1 增量点亮] 已成功向边缘网关提交 {len(d1_batch)} 首曲目！")
                    d1_batch = []
                except Exception as e:
