...
"""
==============================================================================
MOODY - 郑智化 52 首异常曲目高保真录音室母带重制与 AI 听音二次质检流水线
==============================================================================
原则：
1. 定向权威源检索：网易云/酷我/YouTube Topic，严惩古筝民乐翻奏 (Zither Harp) 与纯伴奏；
2. FFmpeg EBU R128 (-14 LUFS) 响度标准化 + 160k CBR Xing MP3 压制；
3. 毫秒级时间轴 LRC 歌词精准抓取与对齐；
4. Groq Whisper 大模型双重切片听音质检（彻底杜绝古筝翻奏、李宗盛串歌、反向水手/南台湾串歌、纯伴奏）；
5. Cloudflare R2 Bucket 07 原位物理覆写（严格守住 9.21 GB 水位，绝不超限 9.5 GB）；
...
TASKS_JSON = os.path.join(BASE_DIR, "reports", "zheng_abnormal_52.json")
WORK_DIR = "/tmp/remediate_zheng_batch"
...
def fetch_netease(title: str, album: str, out_raw: str, out_lrc: str):
    ...
    url = f"https://music.163.com/api/search/get/web?s=郑智化+{title}&type=1&limit=6"
    try:
...
def fetch_kuwo(title: str, album: str, out_raw: str, out_lrc: str):
    clean_t = clean_name(title)
    url = f"http://search.kuwo.cn/r.s?client=kt&all=郑智化+{title}&ft=music&cluster=0&strategy=2012&encoding=utf8&rformat=json&vipver=1&issubtitle=1&show_copyright_off=1&pn=0&rn=6"
    try:
        ...
        for item in d.get("abslist", []):
            ...
            if "郑智化" not in art:
                continue
            if any(k in sname for k in ["伴奏", "纯音乐", "instrumental", "伴唱", "古筝", "Zither"]):
                continue
            if clean_t not in clean_sn and clean_sn not in clean_t:
                continue
...
def fetch_youtube(title: str, out_raw: str):
    queries = [
        f"ytsearch5:Zheng Zhihua {title} 官方",
        f"ytsearch5:郑智化 {title} 官方"
    ]
    for q in queries:
        cmd_search = [
        ...
        try:
            ...
            for line in res.stdout.splitlines():
                ...
                if len(parts) >= 3:
                    ...
                    ch_l = channel.lower()
                    vt_l = vtitle.lower()
                    if ("zheng zhihua" in ch_l or "郑智化" in channel or "郑智化" in vtitle) \
                       and not any(k in vt_l for k in ["伴奏", "instrumental", "karaoke", "zither"]):
...
def verify_whisper(audio_path: str, song_title: str, lrc_path: str = ""):
    ...
    # 黄金切片：35s ~ 65s (30秒) 避开前奏器乐，精准捕捉主唱
    subprocess.run(
        ["ffmpeg", "-y", "-ss", "00:00:35", "-t", "30", "-i", audio_path, "-ac", "1", "-ar", "16000", clip_path],
    ...
    if "水手" not in song_title and any(k in heard_text for k in ["风雨中这点痛算什么", "吹动脸庞的感觉像父亲"]):
        return False, f"听辨到反向串歌《水手》: {heard_text}"
    if "南台湾" not in song_title and "南台湾呀南台湾" in heard_text:
        return False, f"听辨到反向串歌《南台湾》: {heard_text}"
...
def main():
    ...
    for idx, item in enumerate(tasks, 1):
        ...
        s3_key_audio = f"music/郑智化/{album}/s_{sid}.mp3"
        s3_key_lrc = f"lyrics/郑智化/{album}/s_{sid}.lrc"
    ...
    print(f"\n==================================================")
    print(f" 🎉 郑智化第二批次重制完成！成功: {success_count}, 失败: {fail_count}")
    print(f"==================================================")
